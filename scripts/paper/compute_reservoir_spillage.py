...
for scenario_name, scenario in scenarios.items():
    ...
    for area in NO_AREAS:
        try:
            total_spillage = scenario.get_reservoir_spill_for_busbar(area)
            total_discharge = scenario.get_reservoir_discharge_for_busbar(area)
            ...
            if total_spillage is not None and total_discharge is not None:
                ...
                logger.info(
                    ...
                    f"Spillage%={spillage_percentage:.2f}%"
                )
            else:
