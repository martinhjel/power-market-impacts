...
baseline_display = baseline_short

GROUP_CANDIDATES = {
    "N": ["N-LLPS+", "N-BA+", "LLPS_N", "BA_N"],
    "OWN": ["OWN-LLPS+", "OWN-BA+", "LLPS_OWN", "BA_OWN"],
    "OW": ["OW-LLPS+", "OW-BA+", "LLPS_OW", "BA_OW"],
}
SCENARIO_GROUPS = {
    group: [scenario for scenario in GROUP_CANDIDATES[group] if scenario in surplus_results]
    or [scenario for scenario in SCENARIO_GROUPS.get(group, []) if scenario in surplus_results]
    for group in ["N", "OWN", "OW"]
}
...
def _display_scenario_name(scenario_name: str) -> str:
    return baseline_display if scenario_name == baseline_short else scenario_name
...
\midrule
"""
latex_table_norway = latex_table_norway.replace("Baseline in", f"Baseline ({baseline_display}) in")
...
if baseline_short in surplus_results:
    ...
    latex_table_norway += f"\\multicolumn{{1}}{{l}}{{\\texttt{{{baseline_display}}}}} & {cs_base:.1f} & {ps_base:.1f} & {ss_base:.1f} \\\\\n"
...
\midrule
"""
latex_table_nordic = latex_table_nordic.replace("Baseline in", f"Baseline ({baseline_display}) in")
...
if baseline_short in surplus_results:
    ...
    latex_table_nordic += f"\\multicolumn{{1}}{{l}}{{\\texttt{{{baseline_display}}}}} & {cs_base:.1f} & {ps_base:.1f} & {ss_base:.1f} \\\\\n"
...
if baseline_short in technology_surplus_results:
    ...
    latex_tech_table += (
        f"\\texttt{{{baseline_display}}} & {base_tech['hydro']:.0f} & {base_tech['solar']:.0f} & "
