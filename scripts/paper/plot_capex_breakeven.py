"""
Plot bar chart of inferred CAPEX for break-even from nuclear and offshore wind revenue analysis.

This script reads the nuclear_offshore_revenue.csv file and creates visualizations
showing the inferred CAPEX values for different scenarios, technologies, and areas.
"""
...
def _to_short_scenario(name: str) -> str:
    # Already short format.
    if "-" in name and (
        name.startswith(("N-", "OWN-", "OW-", "SMR", "LMR")) or name in {"B", "B+"}
    ):
        return name
    direct = {
        "LLPS_N_UPRATE_": "N-LLPS+",
        "LLPS_OWN_UPRATE_": "OWN-LLPS+",
        "LLPS_OW_UPRATE_": "OW-LLPS+",
        "BA_N_UPRATE_": "N-BA+",
        "BA_OWN_UPRATE_": "OWN-BA+",
        "BA_OW_UPRATE_": "OW-BA+",
        "LLPS_N_FalseHYD_": "N-LLPS",
        "LLPS_OWN_FalseHYD_": "OWN-LLPS",
        "LLPS_OW_FalseHYD_": "OW-LLPS",
        "BA_N_FalseHYD_": "N-BA",
        "BA_OWN_FalseHYD_": "OWN-BA",
        "BA_OW_FalseHYD_": "OW-BA",
    }
    for prefix, short in direct.items():
        if name.startswith(prefix):
            return short
    m = re.match(r"^(SMR\d+|LMR\d+)(BA|LLPS)_", name)
    if m:
        return f"{m.group(1)}-{m.group(2)}"
    return name


df["scenario"] = df["scenario"].astype(str).map(_to_short_scenario)
...
# Calculate average CAPEX per scenario
scenario_capex = df.groupby(['capacity', 'load_type']).agg({
    'breakeven_capex_eur_per_kw': 'mean'
}).reset_index()
...
for cap in capacity_levels:
    ba_val = scenario_capex[(scenario_capex['capacity'] == cap) & 
                            (scenario_capex['load_type'] == 'BA')]['breakeven_capex_eur_per_kw']
    llps_val = scenario_capex[(scenario_capex['capacity'] == cap) & 
                              (scenario_capex['load_type'] == 'LLPS')]['breakeven_capex_eur_per_kw']
...
ax.set_ylabel('Inferred CAPEX for Break-even (EUR/kW)', fontsize=12, fontweight='bold')
ax.set_title('SMR/LMR Inferred CAPEX for Break-even: BA vs LLPS Load Scenarios', 
...
# Save figure
output_file = output_path / "smr_lmr_capex_breakeven.pdf"
...
max_capex = df['breakeven_capex_eur_per_kw'].max()
y_max = max_capex * 1.1  # Add 10% margin
...
if not smr_data.empty:
    pivot_smr = smr_data.pivot_table(
        index='scenario',
        columns='area',
        values='breakeven_capex_eur_per_kw',
        fill_value=0
    )
    ...
    ax_smr.set_ylabel('Inferred CAPEX for Break-even (EUR/kW)', fontsize=12, fontweight='bold')
    ax_smr.set_title('SMR Inferred CAPEX for Break-even by Scenario and Area', 
                     fontsize=14, fontweight='bold')
...
if not lmr_data.empty:
    ...
    ax_lmr.set_ylabel('Inferred CAPEX for Break-even (EUR/kW)', fontsize=12, fontweight='bold')
    ax_lmr.set_title('LMR Inferred CAPEX for Break-even by Scenario and Area', 
                     fontsize=14, fontweight='bold')
...
output_file2 = output_path / "smr_lmr_capex_by_area.pdf"
plt.savefig(output_file2, format='pdf', bbox_inches='tight')
print(f"Saved detailed figure to {output_file2}")

# Print summary statistics
print("\n" + "="*80)
print("SMR/LMR CAPEX SUMMARY STATISTICS")
print("="*80)

summary = df.groupby('scenario').agg({
    'breakeven_capex_eur_per_kw': ['mean', 'min', 'max']
}).round(0)
