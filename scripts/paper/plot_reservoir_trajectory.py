...
def _mean_for_week(values: np.ndarray | pd.Series, week_index: np.ndarray, week: int) -> float:
...
def plot_reservoir_trajectory(
    ...
    # Time steps are weekly in this workflow. Map index to ISO-like week number 1..52.
    ...
    if historical_data:
        ...
        hist_w15 = _mean_for_week(historical_data["mean"], week_index, 15)
        hist_w40 = _mean_for_week(historical_data["mean"], week_index, 40)
        ax.plot(
            time_steps,
            historical_data["mean"],
            color="black",
            linewidth=2.0,
            label=f"Historical Mean (w15={hist_w15:.1f}%, w40={hist_w40:.1f}%)",
            linestyle="-",
            alpha=0.7,
        )
    ...
    for label, data in scenario_data.items():
        ...
        w15 = _mean_for_week(mean_values, week_index, 15)
        w40 = _mean_for_week(mean_values, week_index, 40)
        stats_text += (
            f"{label}:\n"
